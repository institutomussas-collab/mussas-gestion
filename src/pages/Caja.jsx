...
import { Wallet, ArrowUpCircle, ArrowDownCircle, Plus, DollarSign, Lock, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h2 style={{ margin: 0, color: '#00bfa5' }}>Caja Diaria & Arqueo del Mostrador</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '25px' }}>
        <div style={{ padding: '15px', backgroundColor: '#00332c', borderRadius: '8px', borderLeft: '4px solid #00bfa5' }}>
          <small style={{ color: '#aaa' }}>Saldo Inicial del Día ($)</small>
          <input 
            type="number" 
            value={saldoInicial} 
            onChange={e => setSaldoInicial(Number(e.target.value))} 
            style={{ width: '100%', padding: '6px', backgroundColor: '#1e1e1e', color: '#00bfa5', border: '1px solid #333', borderRadius: '4px', marginTop: '5px', fontWeight: 'bold', fontSize: '16px' }}
          />
        </div>

        <div style={{ padding: '15px', backgroundColor: '#1e1e1e', borderRadius: '8px', borderLeft: '4px solid #00e676' }}>
          <small style={{ color: '#aaa' }}>Ingresos Efectivo Total</small>
          <h3 style={{ margin: '5px 0 0 0', color: '#00e676' }}>${(ingresosEfectivoAuto + ingresosEfectivoManual).toLocaleString()}</h3>
        </div>
        ...
        <div style={{ padding: '15px', backgroundColor: '#3e2723', borderRadius: '8px', borderLeft: '4px solid #ff5252' }}>
          ...
          <h3 style={{ margin: '5px 0 0 0', color: '#ff5252' }}>${egresosEfectivoManual.toLocaleString()}</h3>
        </div>
        ...
        <div style={{ padding: '15px', backgroundColor: '#002171', borderRadius: '8px', borderLeft: '4px solid #448aff' }}>
          ...
          <h3 style={{ margin: '5px 0 0 0', color: '#448aff' }}>${ingresosTransfAuto.toLocaleString()}</h3>
        </div>
      </div>

      {/* Arqueo de Caja Mostrador */}
      <div style={{ backgroundColor: '#1e1e1e', padding: '20px', borderRadius: '8px', border: '1px solid #2a2a2a', marginBottom: '25px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', alignItems: 'center' }}>
          <div>
            ...
            <strong style={{ fontSize: '22px', color: '#00bfa5' }}>${totalEfectivoEsperado.toLocaleString()}</strong>
          </div>
          <div>
            ...
            <input 
              ...
              style={{ width: '100%', padding: '10px', backgroundColor: '#282828', color: '#fff', border: '1px solid #444', borderRadius: '5px', fontSize: '16px', marginTop: '4px' }}
            />
          </div>
          <div>
            {diferenciaCaja !== null && (
              <div style={{ padding: '10px', borderRadius: '6px', backgroundColor: diferenciaCaja === 0 ? '#004d40' : diferenciaCaja > 0 ? '#00332c' : '#3e2723', border: '1px solid #444' }}>
                <small style={{ color: '#aaa' }}>Diferencia de Caja:</small>
                <div style={{ fontSize: '18px', fontWeight: 'bold', color: diferenciaCaja === 0 ? '#00e676' : diferenciaCaja > 0 ? '#00e676' : '#ff5252' }}>
      ...
      {/* Grilla de Movimientos Manuales */}
      <h3 style={{ color: '#00bfa5', marginBottom: '10px' }}>Egresos e Ingresos Manuales del Día</h3>
      <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: '#1e1e1e', borderRadius: '8px', overflow: 'hidden' }}>
        <thead>
          <tr style={{ borderBottom: '2px solid #004d40', color: '#00bfa5' }}>
          ...
          {movimientos.length === 0 ? (
            ...
            movimientos.map(m => (
              <tr key={m.id} style={{ borderBottom: '1px solid #2a2a2a' }}>
                <td style={{ padding: '10px', fontWeight: 'bold', color: m.tipo === 'INGRESO' ? '#00e676' : '#ff5252' }}>{m.tipo}</td>
                <td style={{ padding: '10px' }}>{m.categoria}</td>
                <td style={{ padding: '10px' }}>{m.concepto}</td>
                <td style={{ padding: '10px' }}>{m.metodo}</td>
                <td style={{ padding: '10px', fontWeight: 'bold', color: m.tipo === 'INGRESO' ? '#00e676' : '#ff5252' }}>${m.monto.toLocaleString()}</td>
              </tr>
            ))
          )}
        </tbody>
      ...
      {/* Modal Carga Movimiento */}
      {modalAbierto && (
        <div style={modalOverlay}>
          <div style={modalContent}>
            <h3 style={{ color: '#00bfa5', marginTop: 0 }}>Cargar Movimiento Manual</h3>
